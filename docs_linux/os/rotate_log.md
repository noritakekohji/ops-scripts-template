...
```bash
./scripts_linux/os/rotate_log.sh -p /var/log/myapp/app.log -s 100 -c -k 7
```
...
### cron 例（毎日 03:00）
```cron
0 3 * * *  /opt/ops-scripts/scripts_linux/os/rotate_log.sh -L /etc/ops-scripts/logs.txt -a 1 -c -k 30 >> /var/log/ops/rotate.log 2>&1
```
...
[Service]
Type=oneshot
ExecStart=/opt/ops-scripts/scripts_linux/os/rotate_log.sh -L /etc/ops-scripts/logs.txt -a 1 -c -k 30
```
