...
.PARAMETER HtmlReport
    Path for the HTML report file. If omitted, only console output.

.PARAMETER Category
    Categories to compare. Default: all available.
...
function Compare-Os($b, $a) {
    Compare-Dict (Obj-To-Dict $b) (Obj-To-Dict $a) 'os'
}
...
$($catHtml -join "`n")
<div class="footer">Compare-ServerInfo.ps1 &bull; $gen</div>
<script>
...
try {
    ...
    foreach ($r in $allResults) { Write-CategoryConsole $r $DiffOnly.IsPresent }
    ...
    if ($HtmlReport) {
        ...
        $html = New-HtmlReport -bMeta $bMeta -aMeta $aMeta -AllResults $allResults
