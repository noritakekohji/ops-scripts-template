...
# ============================================================================
# mysqlctl.sh
#   MySQL / MariaDB ライフサイクル統合制御: start / stop / restart / status（1本で）
#   （Linux / Bash 版）
#
# 使い方:
#   mysqlctl.sh <action> <service_name> [-w] [-t <sec>]
#
# サービス名の例:
#   mysql       # MySQL (RHEL/CentOS/Debian)
#   mysqld      # MySQL (一部 RHEL 系)
#   mariadb     # MariaDB
#
# アクション:
...
#   -h  usage 表示
#
# 挙動オプションは config/<env>/mysqlctl.conf に設定可能。
...
if [[ "$action" == "status" ]]; then
    ...
    db_ver=$(mysql --version 2>/dev/null | head -1 || mysqld --version 2>/dev/null | head -1 || echo "")
