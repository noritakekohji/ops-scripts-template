...
# ============================================================================
# nginxctl.sh
#   nginx ライフサイクル統合制御: start / stop / restart / status（1本で）
#
# 使い方:
#   nginxctl.sh <action> <service_name> [-w] [-t <sec>]
#
# アクション:
#   start    既に active ならスキップ。それ以外は systemctl start
#   stop     既に inactive ならスキップ。それ以外は systemctl stop
#   restart  systemctl restart（直に実行。停止スキップなし）
...
#   -h  usage 表示
#
# 挙動オプションは config/<env>/nginxctl.conf に設定可能。
