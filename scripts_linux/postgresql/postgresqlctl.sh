...
# ============================================================================
# postgresqlctl.sh
#   PostgreSQL ライフサイクル統合制御: start / stop / restart / status（1本で）
#   （Linux / Bash 版）
#
# 使い方:
#   postgresqlctl.sh <action> <service_name> [-w] [-t <sec>]
#
# サービス名の例:
#   postgresql             # 汎用（RHEL/CentOS 系）
#   postgresql-16          # バージョン指定（RHEL 系）
#   postgresql@16-main     # Debian/Ubuntu 系
#
# アクション:
...
# オプション:
#   -w  目的状態到達まで待機
#   -t  待機タイムアウト秒（既定: 120、許容範囲 5..600）
#   -h  usage 表示
#
# 挙動オプションは config/<env>/postgresqlctl.conf に設定可能。
...
# サービス名: postgresql, postgresql-16, postgresql@16-main などを許容
...
if [[ "$action" == "status" ]]; then
    ...
    db_ver=$(psql --version 2>/dev/null | head -1 || echo "")
    log_info "Status: service=$service_name state=$before_state sub=$sub_state version='$db_ver'"
