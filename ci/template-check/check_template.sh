...
# ============================================================================
# check_template.sh
#   Verify that every script under scripts/ conforms to the template /
#   shell-specification.md rules.
#
...
# --- PowerShell scripts -----------------------------------------------------
echo "==> Checking PowerShell scripts under scripts/..." >&2
while IFS= read -r -d '' f; do
    ps_count=$((ps_count + 1))
    require_pattern    "$f" "PS: must declare #Requires -Version 7"          '^#Requires -Version 7'
    ...
    require_pattern    "$f" "PS: must import lib/powershell/Logging.psm1"    'Logging\.psm1'
done < <(find "$REPO_ROOT/scripts" -type f -name '*.ps1' -print0 2>/dev/null || true)

# --- Bash scripts -----------------------------------------------------------
echo "==> Checking Bash scripts under scripts/..." >&2
while IFS= read -r -d '' f; do
    sh_count=$((sh_count + 1))
    ...
    require_pattern    "$f" "Bash: must source lib/bash/logging.sh"               'lib/bash/logging\.sh'
    if [[ ! -x "$f" ]]; then
        violation "$f" "Bash: must be executable (chmod +x / git update-index --chmod=+x)"
    fi
done < <(find "$REPO_ROOT/scripts" -type f -name '*.sh' -print0 2>/dev/null || true)
