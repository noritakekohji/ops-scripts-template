...
| OS | スクリプト |
|---|---|
| Windows | `scripts/aws/windows/Ec2Ctl.ps1` |
| Linux | `scripts/aws/linux/ec2ctl.sh` |
...
### cron（業務時間に合わせて起動・停止）
```cron
0  9 * * 1-5  OPS_ENV=prd /opt/ops-scripts/scripts/aws/linux/ec2ctl.sh start i-0abc,i-0def -w
0 21 * * 1-5  OPS_ENV=prd /opt/ops-scripts/scripts/aws/linux/ec2ctl.sh stop  i-0abc,i-0def -w
```
