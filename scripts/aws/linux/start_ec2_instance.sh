...
#   Starts one or more EC2 instances. Idempotent: already-running instances
#   are skipped without error.
#
# Usage:
#   start_ec2_instance.sh -i <instance-ids> [-r <region>] [-w] [-t <sec>]
#
# Options:
#   -i  Comma-separated EC2 instance IDs (required, e.g. i-0abc,i-0def)
#   -r  AWS region (default: from environment / profile / config)
#   -w  Wait until every started instance reaches 'running'
...
#             4 start failed, 10 aws missing, 20 auth
# ============================================================================
set -euo pipefail
...
declare -a started=()
declare -a skipped_running=()
status="unknown"

cleanup() {
    local rc=$?
    if [[ "$status" == "unknown" && "$rc" -eq 0 ]]; then status="success"; fi
    log_info "Script end: status=$status exitCode=$rc started=${#started[@]} skippedRunning=${#skipped_running[@]}"
}
trap cleanup EXIT
...
# --- Phase 2: load config ---------------------------------------------------
load_ops_config "start_ec2_instance"
...
log_info "Args validated: instanceCount=${#instance_ids[@]} region=${region:-default} wait=$wait_for_completion timeoutSec=$wait_timeout"
...
while IFS=$'\t' read -r iid st; do
    ...
    case "$st" in
        ...
        stopped)
            ...
            ;;
        stopping)
            log_warn "Instance is stopping; cannot start now: instanceId=$iid"
            ;;
...
if [[ "${#to_start[@]}" -eq 0 ]]; then
    log_info "Skipped (idempotent): reason=all_already_running count=${#skipped_running[@]}"
    status="skipped"; exit 0
fi

log_info "Pre-check passed: toStart=${#to_start[@]} skippedRunning=${#skipped_running[@]}"

# --- Phase 4: main processing -----------------------------------------------
log_info "Main start"

if ! aws ec2 start-instances --instance-ids "${to_start[@]}" "${region_arg[@]}" >/dev/null; then
    log_error "Start API call failed"
    status="failed"; exit 4
fi
started=( "${to_start[@]}" )
log_info "Start initiated: instanceIds=$(IFS=,; echo "${started[*]}") count=${#started[@]}"

if [[ "$wait_for_completion" -eq 1 ]]; then
    log_info "Waiting for 'running': count=${#started[@]} timeoutSec=$wait_timeout"
    if ! timeout "$wait_timeout" aws ec2 wait instance-running --instance-ids "${started[@]}" "${region_arg[@]}"; then
        log_error "Did not reach 'running' within timeout: timeoutSec=$wait_timeout"
        status="failed"; exit 3
    fi
    log_info "All instances are running"
fi
