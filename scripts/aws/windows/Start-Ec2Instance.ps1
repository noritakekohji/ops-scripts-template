...
<#
.SYNOPSIS
    Starts one or more EC2 instances. Idempotent: already-running
    instances are skipped without error.

.DESCRIPTION
    Reads behavior parameters from config files (Region / Wait /
    WaitTimeoutSec). Per-run targets (-InstanceId) are CLI-only.

    Idempotency:
      - running / pending : skipped (already in desired state)
      - stopped           : started
      - stopping          : warned (cannot transition stopping -> running)
      - shutting-down / terminated : exit 3 (invalid state)

    Authentication: relies on the default AWS credential chain.

.PARAMETER InstanceId
    One or more EC2 instance IDs. Pass multiple via comma:
        -InstanceId i-0abc,i-0def

.PARAMETER Region
    AWS region. Falls back to default region from env / profile / config.

.PARAMETER Wait
    Wait until every started instance reaches 'running'. Skipped instances
    do not contribute to the wait list.

.PARAMETER WaitTimeoutSec
    Maximum seconds to wait when -Wait is set. Default: 600 (10 min).
...
$cfg = Get-OpsConfig -Name 'Start-Ec2Instance'
...
try {
    do {
        ...
        Write-OpsLog -Level INFO -Message "Args validated: instanceCount=$($InstanceId.Count) region=$Region wait=$Wait timeoutSec=$WaitTimeoutSec"
        ...
        foreach ($inst in $instances) {
            ...
            switch ($state) {
                'running'       { $skippedRunning += $inst.InstanceId; Write-OpsLog -Level INFO  -Message "Skipped (idempotent): instanceId=$($inst.InstanceId) state=running" }
                'pending'       { $skippedRunning += $inst.InstanceId; Write-OpsLog -Level INFO  -Message "Skipped (idempotent): instanceId=$($inst.InstanceId) state=pending" }
                'stopped'       { $toStart        += $inst.InstanceId }
                'stopping'      { Write-OpsLog -Level WARN  -Message "Instance is stopping; cannot start now: instanceId=$($inst.InstanceId)" }
        ...
        if ($toStart.Count -eq 0) {
            Write-OpsLog -Level INFO -Message "Skipped (idempotent): reason=all_already_running count=$($skippedRunning.Count)"
            $exitCode = 0; $status = 'skipped'; break
        }

        Write-OpsLog -Level INFO -Message "Pre-check passed: toStart=$($toStart.Count) skippedRunning=$($skippedRunning.Count)"

        # --- Phase 4: main processing ---------------------------------------
        Write-OpsLog -Level INFO -Message 'Main start'

        if ($PSCmdlet.ShouldProcess(($toStart -join ','), 'Start EC2 instance(s)')) {
            $result = Start-EC2Instance -InstanceId $toStart @aws
            $started = @($result | ForEach-Object { $_.InstanceId })
            Write-OpsLog -Level INFO -Message "Start initiated: instanceIds=$($started -join ',') count=$($started.Count)"
        }

        if ($Wait -and $started.Count -gt 0) {
            Write-OpsLog -Level INFO -Message "Waiting for 'running': count=$($started.Count) timeoutSec=$WaitTimeoutSec"
            $deadline = (Get-Date).AddSeconds($WaitTimeoutSec)
            while ((Get-Date) -lt $deadline) {
                $current = @((Get-EC2Instance -InstanceId $started @aws).Instances)
                $pending = @($current | Where-Object { $_.State.Name -ne 'running' })
            ...
            $final = @((Get-EC2Instance -InstanceId $started @aws).Instances)
            $notRunning = @($final | Where-Object { $_.State.Name -ne 'running' })
            if ($notRunning.Count -gt 0) {
                Write-OpsLog -Level ERROR -Message "Did not reach 'running' within timeout: instanceIds=$(($notRunning.InstanceId -join ',')) timeoutSec=$WaitTimeoutSec"
                $exitCode = 3; $status = 'failed'; break
            }
            Write-OpsLog -Level INFO -Message 'All instances are running'
        }
    ...
    Write-OpsLog -Level INFO -Message "Script end: status=$status exitCode=$exitCode started=$($started.Count) skippedRunning=$($skippedRunning.Count)"
}
