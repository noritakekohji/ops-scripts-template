...
- HTTP check uses `Invoke-WebRequest -UseBasicParsing`.
- ICMP check uses `System.Net.NetworkInformation.Ping`.
- TCP check uses `System.Net.Sockets.TcpClient.ConnectAsync` with `Wait(TimeSpan)`.
...
| 2 | List parse error |
| 3 | Timeout |
| 10 | Prerequisite missing (only emitted by the bat launcher when `ServiceWait.ps1` is absent; PowerShell itself uses .NET built-ins so has no external prereq) |
