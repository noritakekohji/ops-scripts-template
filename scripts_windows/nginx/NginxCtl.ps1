...
<#
.SYNOPSIS
    nginx ライフサイクル統合制御：start / stop / restart / status を 1 本で。Windows / Linux 共通仕様。
.DESCRIPTION
    nginx ライフサイクル統合制御：start / stop / restart / status を 1 本で行います。
    Windows サービス（nssm 等で登録された nginx サービス）を制御することを前提とし、
    Linux 版 (nginxctl.sh) と同じコマンド体系・終了コード規約を持ちます。

    Usage:
      NginxCtl.ps1 <action> <service_name> [-Wait] [-WaitTimeoutSec N]
    ...
    config/<env>/nginxctl.conf.

.EXAMPLE
    .\NginxCtl.ps1 start nginx -Wait
    .\NginxCtl.ps1 stop  nginx
    .\NginxCtl.ps1 restart nginx -Wait
    .\NginxCtl.ps1 status nginx
#>
...
try {
    do {
        ...
        if (-not $PSCmdlet.ShouldProcess($ServiceName, "nginx $Action")) { break }
