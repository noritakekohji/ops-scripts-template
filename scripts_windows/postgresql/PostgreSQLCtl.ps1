...
<#
.SYNOPSIS
    PostgreSQL ライフサイクル統合制御：start / stop / restart / status を 1 本で。Windows / Linux 共通仕様。
.DESCRIPTION
    PostgreSQL ライフサイクル統合制御：start / stop / restart / status を 1 本で。Windows / Linux 共通仕様。

    Windows サービスとして稼働する PostgreSQL を
    Get-Service / Start-Service / Stop-Service / Restart-Service で制御する。

    サービス名の例:
      - postgresql-x64-16   (PostgreSQL 16, 64-bit)
      - postgresql-x64-15   (PostgreSQL 15, 64-bit)
      - postgresql           (古いバージョン / 手動インストール)

    Usage: PostgreSQLCtl.ps1 <action> <service_name> [-Wait] [-WaitTimeoutSec N]
...
.EXAMPLE
    .\PostgreSQLCtl.ps1 start  postgresql-x64-16 -Wait
    .\PostgreSQLCtl.ps1 stop   postgresql-x64-16 -Wait -WaitTimeoutSec 60
    .\PostgreSQLCtl.ps1 restart postgresql-x64-16
    .\PostgreSQLCtl.ps1 status  postgresql-x64-16
#>
...
$cfg = Get-OpsConfig -Name 'postgresqlctl'
...
try {
    do {
        ...
        if ($Action -eq 'status') {
            # psql.exe のバージョン情報も付加（インストールされている場合）
            $pgVer = ''
            $psqlExe = Get-Command psql -ErrorAction SilentlyContinue
            if ($psqlExe) {
                $pgVer = (& $psqlExe.Source --version 2>$null) -join ''
            }
            Write-OpsLog -Level INFO -Message "Status: service=$ServiceName state=$beforeState startType=$($svc.StartType) displayName='$($svc.DisplayName)' version='$pgVer'"
        ...
        if (-not $PSCmdlet.ShouldProcess($ServiceName, "PostgreSQL $Action")) { break }
